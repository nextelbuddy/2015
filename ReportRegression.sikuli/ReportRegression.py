...
#-------------This test will navigate the report list and open specific most used reports - 
#-------------as well as print each report to all of its respective output devices
...
def ReportPrintTAB():
    ...
    if exists(Pattern("Qpen.png").similar(0.79)):
        type('o', KeyModifier.ALT)
    wait(.5)
    type(Key.ENTER)
    if exists(Pattern("Doyouwanttor-1.png").similar(0.91), 30):
...
def ReportSupportsOutput(pGroup, pIndex, pOutput):
    if (pGroup == 1) and (pIndex == 6) and (pOutput == 5):
        return False
    else:
        return True"""
